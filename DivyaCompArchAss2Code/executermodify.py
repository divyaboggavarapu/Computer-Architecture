...
if bzpred == None:
    ...
    defaultprediction = [defaultprediction] * bzpred
    
scoreboard = []
scoreboard = {-1: [None for i in range(0,numregs)] }
instructions = {-1: (None, None, None, None, defaultprediction)}
...
def loadmem():                                       # get binary load image
  curaddr = 0
  for line in open("/Users/divya/Desktop/CompArch/DivyaCompArchAssignment2/DivyaCompArchAss2Code/a.out", 'r').readlines():
...
while( 1 ):
   ...
   clock += 4                                       # 4 clock cycles for fetch
   ip = ip + 1
   ...
   def decode(ir):
    ...
    reg2   = (ir >> reg2position) & regmask
    addr   = (ir) & addmask
    return True, opcode, reg1, reg2, addr 
   status, newopcode, reg1, reg2, addr = decode(ir)
   def setscoreboardline(scoreboardline, r, v):
      if ( (r & (1<<numregbits)) == 0 ):               # not indirect
         scoreboardline[r] = v
      else:
         scoreboardline[r - numregs] = v
      return scoreboardline

   if newopcode:
        scoreboardline = [None for i in range(0,numregs)]
        if opcodes[ newopcode ] [0] == 1:                   #     dec, inc, ret type
            setscoreboardline(scoreboardline,reg1,"R/W")
        elif opcodes[ newopcode ] [0] == 2:                 #     add, sub type
            setscoreboardline(scoreboardline,reg2,"R")
            setscoreboardline(scoreboardline,reg1,"R/W")
        elif opcodes[ newopcode ] [0] == 3:                 #     ld, st, br type
            if (newopcode == 7) or (newopcode == 9):
                setscoreboardline(scoreboardline,reg1,"R/W")
            else:
                setscoreboardline(scoreboardline,reg1,"R")              
        scoreboard = scoreboardline
        instructions = (newopcode, reg1, reg2, addr, defaultprediction)
   if (newopcode == 12) or (newopcode == 13):
      if bzpred == None:
         pass
      elif bzpred == -1:
         if addr < ip[-1]:
...
def dumpscoreboard():
    ...
    for k in scoreboard:
        ins = instructions
        regstatus = scoreboard
        print("line no: {} opcode: {} reg1: {} reg2: {} addr: {} bzpred: {} regstatus: {} ".format(k,opcodes.get(ins[0],None),ins[1], ins[2], ins[3], ins[4], regstatus))
