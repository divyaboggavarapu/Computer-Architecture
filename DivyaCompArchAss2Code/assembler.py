...
opcodes = {'add': (2, 1),             # ie, "add" is a type 2 instruction, opcode = 1
           'sub': (2, 2),              
           'dec': ( 1, 3), 
           'inc': ( 1, 4 ), 
           'ld': (3, 7), 
           ...
           'ret': ( 1, 14 ),
           'int': (3, 16), 
           'sys': (3, 16),             # syscalls are same as interrupts
           'dw': (4, 0),
           'go':(3, 0), 
           'end': (0, 0) }  # pseudo ops
curaddr = 0                                            # start assembling to location 0
#for line in open(sys.argv[1], 'r').readlines():       # command line
infile = open("/Users/divya/Desktop/CompArch/DivyaCompArchAssignment2/DivyaCompArchAss2Code/in.asm", 'r')
# Build Symbol Table
symboltable = {}
...
for line in infile.readlines():           # read our asm code
   ...
   if instype == 4:                                        # dw type
      ...
      token2 = tokens[2]
      if token2[0] == '*':
        memdata = memdata + (regval(tokens[2]) << reg2position) + (regval(tokens[1]) << reg1position) 
      elif token2.isdigit():
         memaddr = int( tokens[2] )
         memdata = memdata + memaddr + ( regval( tokens[1] ) << reg1position ) 
      else:
   ...
   curaddr = curaddr + 1
outfile = open("/Users/divya/Desktop/CompArch/DivyaCompArchAssignment2/DivyaCompArchAss2Code/a.out", 'w')                               # done, write it out
